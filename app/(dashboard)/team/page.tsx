...
export default function TeamPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", role: "member" as const, department: "", position: "", phone: "", bio: "", skills: "" });
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) { await update(editing.id, data); toast.success("Member updated"); }
      else { await add(data); toast.success("Member added"); }
      setShowModal(false);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {members.length === 0 ? (
        <EmptyState title="No team members" message="Add your first team member to get started." action={<button onClick={openAdd} className="btn-primary"><Plus className="w-4 h-4" /> Add Member</button>} />
      ) : (
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editing ? "Edit Member" : "Add Member"}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="label">Role</label><select className="input-field" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as "admin" | "member" })}><option value="member">Member</option><option value="admin">Admin</option></select></div>
