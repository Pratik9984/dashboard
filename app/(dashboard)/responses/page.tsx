...
export default function ResponsesPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", subject: "", message: "", source: "website" as Response["source"], priority: "medium" as Response["priority"] });

  const openAdd = () => { setSelected(null); setForm({ name: "", email: "", phone: "", subject: "", message: "", source: "website", priority: "medium" }); setShowModal(true); };
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {["all", "new", "read", "replied", "archived"].map((s) => (
            <button key={s} onClick={() => setFilter(s)} className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors ${filter === s ? "bg-primary-600 text-white" : "bg-white text-slate-600 border border-slate-200 hover:bg-slate-50"}`}>
              {s.charAt(0).toUpperCase() + s.slice(1)} {s !== "all" && <span className="text-xs opacity-70">({responses.filter((r) => r.status === s).length})</span>}
            </button>
          ))}
        </div>
        {canAdd && (
